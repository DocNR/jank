...
export function createHighlightDraftEvent(
  ...
  const hint = seenOn.getEventHint(sourceEvent.id)
...
export function createDeckSyncDraftEvent(encryptedContent: string): TDraftEvent {
  return {
    kind: kinds.Application,
    content: encryptedContent,
    tags: [buildDTag(ApplicationDataKey.DECKS), ['alt', 'jank deck layout (private sync)']],
    created_at: dayjs().unix()
  }
}
  ...
  if (parentEvent) {
    ...
    if (_rootTag?.type === 'e') {
      ...
      if (rootEventPubkey) {
        ...
        const rootEvent = rootEventId ? await eventCache.fetchEvent(rootEventId) : undefined
        rootTag = rootEvent
  ...
  for (const m of matches || []) {
    try {
      ...
      if (type === 'nevent') {
        ...
        if (!quoteSet.has(id)) {
          quoteSet.add(id)
          const relay = data.relays?.[0] ?? seenOn.getEventHint(id)
        ...
        if (!quoteSet.has(id)) {
          quoteSet.add(id)
          const relay = seenOn.getEventHint(id)
...
export function buildATag(event: Event, upperCase: boolean = false) {
  ...
  const hint = seenOn.getEventHint(event.id)
...
export function buildETag(
  ...
  if (!hint) {
    hint = seenOn.getEventHint(eventHexId)
  }
...
function buildLegacyRootATag(coordinate: string, hint: string = '') {
  if (!hint) {
    const evt = eventCache.getReplaeableEventFromCache(coordinate)
    if (evt) {
      hint = seenOn.getEventHint(evt.id)
    }
  }
...
function buildClientTag() {
  return ['client', BRAND.nostrClientTag]
}
