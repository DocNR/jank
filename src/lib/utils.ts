 ...
 *
 * `crypto.randomUUID()` is gated to secure contexts (HTTPS or localhost), so
 * it's `undefined` when devs load the dev server from a phone over plain
 * HTTP on the LAN. Falls back to a v4-formatted UUID built from
