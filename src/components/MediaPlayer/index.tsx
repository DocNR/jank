...
export default function MediaPlayer({
  ...
  if (!mediaType) {
    // Reserve aspect-ratio space while probing the URL so the row doesn't
    // jolt when the video/audio metadata loads. Falls back to 16:9 when
    // imeta dim is missing (matches the typical landscape video aspect).
    return (
      <div
        className={cn('bg-muted rounded-xl border', className)}
        style={{
