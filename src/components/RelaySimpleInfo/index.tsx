  ...
  return (
    <div className={cn('space-y-1', className)} {...props}>
      <div className="flex w-full items-start justify-between gap-2">
        <div className="flex w-0 flex-1 items-center gap-2">
          ...
          <div className="w-0 flex-1">
            ...
            {relayInfo?.name && (
              <div className="text-muted-foreground truncate text-xs">{relayInfo?.shortUrl}</div>
            )}
          </div>
        </div>
      ...
      {!!relayInfo?.description && (
        <div className="line-clamp-3 overflow-hidden wrap-anywhere whitespace-pre-wrap">
      ...
      {!!users?.length && (
        <div className="flex items-center gap-2">
          ...
          <div className="flex items-center gap-1">
            ...
            {users.length > 10 && (
              <div className="bg-muted text-muted-foreground flex h-5 w-5 items-center justify-center rounded-full text-xs">
                +{users.length - 10}
              </div>
            )}
          </div>
