...
function ZapDialogContent({
  ...
  // The zap is initiated by the column's signing identity when inside a column
  // scope, else the global active account.
  const { signerPubkey } = useSigningContext()
  ...
  const isSelfZap = useMemo(() => signerPubkey === recipient, [signerPubkey, recipient])
  ...
  const handleZap = async () => {
    try {
      ...
      setZapping(true)
      const zapResult = await lightning.zap(signerPubkey, event ?? recipient, sats, comment, () =>
        setOpen(false)
      )
      // user canceled
      if (!zapResult) {
      ...
      if (event) {
        stuffStatsService.addZap(signerPubkey, event.id, zapResult.invoice, sats, comment)
      }
    } catch (error) {
  ...
  return (
    <>
      {/* Which account this zap is sent from — mirrors the compose modal's
          "posting as" affordance. Reflects the focused column's signer. */}
      {signerPubkey && (
        <div className="text-muted-foreground flex items-center justify-center gap-1.5 text-xs">
          <span className="shrink-0">{t('Zapping from')}</span>
          <UserAvatar size="xSmall" userId={signerPubkey} />
          <Username userId={signerPubkey} className="font-medium" withoutSkeleton />
        </div>
      )}
