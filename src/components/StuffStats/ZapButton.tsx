...
export default function ZapButton({ stuff }: { stuff: Event | string }) {
  ...
  // Zaps are initiated by the column's signingIdentity when inside a column
  ...
  const { zapAmount, hasZapped } = useMemo(() => {
    return {
      ...
      hasZapped: signerPubkey
        ? noteStats?.zaps?.some((zap) => zap.pubkey === signerPubkey)
        : false
    }
  ...
  useEffect(() => {
    ...
    profileFetcher.fetchProfile(event.pubkey).then((profile) => {
      if (!profile) return
  ...
  const handleZap = async () => {
    try {
      ...
      setZapping(true)
      const zapResult = await lightning.zap(signerPubkey, event, defaultZapSats, defaultZapComment)
      // user canceled
      if (!zapResult) {
        return
      }
