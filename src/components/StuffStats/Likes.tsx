...
export default function Likes({ stuff }: { stuff: Event | string }) {
  ...
  const { signerPubkey, checkLogin, publish } = useSigningContext()
  ...
  const handleMouseDown = (key: string) => {
    if (signerPubkey && likes.find((l) => l.key === key)?.pubkeys.has(signerPubkey)) {
      return
    }
        ...
        {likes.map(({ key, emoji, pubkeys }) => (
          <div
            ...
            className={cn(
              ...
              signerPubkey && pubkeys.has(signerPubkey)
