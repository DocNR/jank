...
const CommandDialog = ({
  ...
  shouldFilter = false,
  ...props
}: DialogProps & { classNames?: { content?: string }; shouldFilter?: boolean }) => {
  return (
    <Dialog {...props}>
      ...
      <DialogContent
        className={cn(
          // Override DialogContent's `top-[50%] [transform:translate(-50%,-50%)]`
          // centering: pin near the top and remove the Y translation. Use the
          // same arbitrary-syntax form to win the twmerge resolution.
          'data-[state=closed]:slide-out-to-top-4 data-[state=open]:slide-in-from-top-4 top-[10vh] [transform:translate(-50%,0)] overflow-hidden p-0 shadow-lg',
          classNames?.content
        )}
      >
        <Command
...
>(({ className, ...props }, ref) => (
  <div className="flex items-center border-b px-3" cmdk-input-wrapper="">
    ...
    <CommandPrimitive.Input
      ref={ref}
      className={cn(
        'placeholder:text-muted-foreground flex h-10 w-full rounded-md bg-transparent py-3 pe-6 text-sm outline-hidden disabled:cursor-not-allowed disabled:opacity-50',
        className
      )}
      {...props}
    />
