/**
 * jank mark. Single-path glyph; `fill` inherits `currentColor` so the mark
 * themes with the surrounding text color (black on light, white on dark).
 *
 * Source canvas is 792×612 but the glyph ink only occupies a 332×281 region
 * starting at (230, 166). Cropping the viewBox to that bbox eliminates the
 * empty padding so callers get a fully-filled, larger-feeling mark at the
 * same `size` value. Aspect ratio: 332:281 ≈ 1.18.
 */
export default function JankMark({
  size = 24,
  className
}: {
  size?: number
