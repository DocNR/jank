...
export function EmbeddedNote({ noteId, className }: { noteId: string; className?: string }) {
  ...
  // Reveal the real card on the first render when the event is already cached (e.g.
  // this row is remounting on a virtualizer scroll-back). Skipping the skeleton on
  // that path avoids the short-skeleton → full-card height jump that lurched the feed.
  // useFetchEvent seeds `event` from the same synchronous cache, so the card renders
  // immediately rather than briefly hitting the not-found branch.
  const [revealed, setRevealed] = useState(() => !!eventCache.getCachedEvent(noteId))
