...
export default function PinnedButton({ event }: { event: NostrEvent }) {
  ...
  if (event.pubkey !== pubkey) {
    return (
      <div className="text-primary mb-1 flex h-fit items-center gap-1 px-4 py-0 text-sm">
  ...
  return (
    <Button
      className="text-primary mb-1 flex h-fit items-center gap-1 px-4 py-0.5 text-sm"
      variant="link"
      onClick={(e) => {
        e.stopPropagation()
        setUnpinning(true)
