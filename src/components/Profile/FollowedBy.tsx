...
export default function FollowedBy({ pubkey }: { pubkey: string }) {
  ...
  useEffect(() => {
    ...
    const init = async () => {
      const followings = (await followListService.fetchFollowings(myPubkey)).reverse()
      const followingsOfFollowings = await Promise.all(
        followings.map(async (following) => {
          return followListService.fetchFollowings(following, false)
        })
      )
