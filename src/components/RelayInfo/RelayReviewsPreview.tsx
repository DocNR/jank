...
export default function RelayReviewsPreview({ relayUrl }: { relayUrl: string }) {
  ...
  useEffect(() => {
    const init = async () => {
      ...
      const events = await eventCache.fetchEvents([relayUrl, ...getDefaultRelayUrls()], filters, {
        cache: true
      })
        ...
        <div>
          ...
          <div
            className={cn(
              ...
              count > 0 && 'hover:text-foreground cursor-pointer underline'
            )}
            onClick={() => {
              if (count > 0) {
    ...
    >
      <CarouselContent className="ms-4 me-2">
        ...
        {reviews.length > 10 && (
          <Item>
            <div
              className="bg-muted/20 hover:bg-muted flex h-full cursor-pointer items-center justify-center rounded-lg border p-3"
              onClick={() => push(toRelayReviews(relayUrl))}
            >
              <div className="text-muted-foreground text-sm">{t('View more reviews')}</div>
            </div>
          </Item>
        )}
      </CarouselContent>
...
function Item({ children }: { children: React.ReactNode }) {
  return (
    <CarouselItem className="basis-11/12 ps-0 pe-2 md:basis-5/12 lg:basis-7/12">
      {children}
    </CarouselItem>
  )
}
