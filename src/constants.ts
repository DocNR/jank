...
// Upstream Jumble's hosted translation API. Value unchanged from fork — jank
// uses Cody's service. Constant name renamed for clarity (the service isn't ours).
export const TRANSLATION_API_BASE_URL = 'https://api.jumble.social'
...
export const StorageKey = {
  ...
  READ_NOTIFICATIONS_MAP: 'readNotificationsMap',
  ...
  // Value preserved as 'decksByAccount' for backward compat; do not migrate.
  WORKSPACES_BY_ACCOUNT: 'decksByAccount',
  ACTIVE_ACCOUNT_PUBKEY: 'activeAccountPubkey',
  DECK_SYNC_META: 'deckSyncMeta',
  TRANSIENT_COLUMN_MODE: 'transientColumnMode',
  DENSITY: 'density',
  DECK_LEADING_GUTTER: 'deckLeadingGutter',
...
export const ApplicationDataKey = {
  ...
  // legacy wire identifier (NIP-78 deck-sync d-tag) — do NOT rename; renaming orphans every user's synced decks
  DECKS: 'spectr_decks'
}
...
export const ExtendedKind = {
  ...
  VOICE: 1222,
  VOICE_COMMENT: 1244,
  PINNED_USERS: 10010, // user-facing label: "Favorites" — wire/storage identifier kept for Nostr protocol compatibility
  FAVORITE_RELAYS: 10012,
  DM_RELAY_LIST: 10050, // NIP-17 preferred DM relays (where a user receives gift-wrapped DMs)
...
export const DEFAULT_FEED_TABS: TFeedTabConfig[] = [
  ...
  { id: 'postsAndReplies', builtin: 'postsAndReplies', label: 'Notes and replies' }
]

// Events dated further than this into the future (skewed author clock, abuse)
// are hidden from feeds until the wall clock catches up — mirrors the common
// relay convention of rejecting events "too far off from the current time".
export const FUTURE_EVENT_TOLERANCE_SECONDS = 15 * 60
...
// CLINK Offer (https://github.com/shocknet/CLINK). The spec defines no URI
// scheme — the raw bech32 string is canonical; the optional nostr: prefix is
// consumed defensively so clients that wrap it never leave a dangling "nostr:".
// 60-char floor: the required TLVs (pubkey + relay + offer id) alone exceed it.
export const NOFFER_REGEX = /(?:nostr:)?noffer1[02-9ac-hj-np-z]{60,}/g
...
// Cody Tseng's pubkey — upstream Jumble donation recipient. Preserved from fork.
export const UPSTREAM_DONATION_PUBKEY =
...
export const PRIMARY_COLORS = {
  DEFAULT: {
    ...
    light: {
      primary: '186 75% 45%',
      'primary-hover': '186 75% 55%',
      'primary-foreground': '186 90% 8%',
      ring: '186 75% 45%'
    },
    dark: {
      primary: '186 75% 45%',
