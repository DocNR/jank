/**
 * Schemata validation tests for jank's draft event builders (inherited from Jumble).
 *
...
vi.mock('@/services/client.service', () => ({
  default: {
    ...
    // Draft builders for event-referencing kinds (reply/repost/reaction/
    // comment/highlight) pull relay hints via client.pool.getSeenRelays();
    // stub it to "no seen relays" so those builders run in the test env.
    pool: {
      getSeenRelays: () => []
    }
  }
...
// indexed-db.service opens a real IndexedDB connection at module load
// (window.indexedDB.open). happy-dom provides `window` but not IndexedDB, so
// the real module throws at import. Mock it out — the draft builders under
// test don't depend on its behavior, only on it loading without crashing.
vi.mock('@/services/indexed-db.service', () => ({
  default: {
    init: () => Promise.resolve(),
    getReplaceableEvent: () => Promise.resolve(undefined),
    getReplaceableEventByCoordinate: () => Promise.resolve(undefined),
    getManyReplaceableEvents: () => Promise.resolve([]),
    getDecryptedContent: () => Promise.resolve(null),
    putDecryptedContent: () => Promise.resolve(),
    getEvents: () => Promise.resolve([]),
    putEvents: () => Promise.resolve()
  }
}))
...
const schemataBase = require_.resolve('@nostrability/schemata').replace(/\/dist\/.*/, '/dist/nips')
...
describe('Schemata Schema Validation', () => {
  ...
  // Kind 6 / kind 7 fill their `e` tag's relay-hint slot (index 2) from
  // seenOn.getEventHint(). NIP-18 requires a relay URL in the repost `e` tag,
  // and the schemata kind-7 `e` tag schema rejects an empty hint slot — so a
  // synthetic event with no seen-on data makes the builders emit
  // ["e", id, "", pubkey], which both schemas reject. Production always has a
  // hint (the target event was fetched from some relay), so each test seeds
  // one. A dedicated event id keeps the seeded hint from leaking into other
  // tests that share makeEvent()'s default id.

  // Kind 6 – Repost (text note)
  it('kind 6 (Repost) via createRepostDraftEvent', () => {
    const event = makeEvent({ kind: 1, id: 'd'.repeat(64) })
    seenOn.trackEventExternalSeenOn(event.id, ['wss://relay.example.com'])
  ...
  it('kind 7 (Reaction: like) via createReactionDraftEvent', () => {
    const event = makeEvent({ id: 'd'.repeat(64) })
    seenOn.trackEventExternalSeenOn(event.id, ['wss://relay.example.com'])
  ...
  it('kind 1111 (Comment on external content) via createCommentDraftEvent', async () => {
    const draft = await createCommentDraftEvent(
      'Interesting article',
  ...
  // Kind 30078 – Application-specific data (encrypted deck layout)
  it('kind 30078 (Application Data) via createDeckSyncDraftEvent', () => {
    const draft = createDeckSyncDraftEvent('ciphertext-placeholder')
    expect(draft.kind).toBe(kinds.Application)
    const result = validateDraftEvent(draft, schemaRegistry)
    expect(result.errors).toEqual([])
    expect(result.valid).toBe(true)
