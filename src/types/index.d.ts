...
export type TThemePreset = 'modern' | 'terminal'
...
export type TNip07 = {
  ...
  nip44v3?: {
    encrypt?: (pubkey: string, plainText: string, kind: number, scope: string) => Promise<string>
    decrypt?: (pubkey: string, cipherText: string, kind: number, scope: string) => Promise<string>
  }
}
...
export interface ISigner {
  ...
  /**
   * Synchronous capability probe for NIP-44 v3. Returns true when this signer
   * can encrypt/decrypt with the context-aware v3 scheme. Bunker signers
   * resolve this via the NIP-46 `describe` RPC at login time; local signers
   * (nsec) always return true; npub view-only signers always return false.
   * Optional for back-compat with signer implementations that predate v3.
   */
  supportsNip44v3?: () => boolean
  nip44v3Encrypt?: (pubkey: string, plainText: string, kind: number, scope: string) => Promise<string>
  nip44v3Decrypt?: (pubkey: string, cipherText: string, kind: number, scope: string) => Promise<string>
}
...
export type TFeedTabBuiltin = 'posts' | 'postsAndReplies'
