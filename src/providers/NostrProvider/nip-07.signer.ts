...
export class Nip07Signer implements ISigner {
  ...
  supportsNip44v3(): boolean {
    return !!(this.signer?.nip44v3?.encrypt && this.signer?.nip44v3?.decrypt)
  }
  ...
  async nip44v3Encrypt(pubkey: string, plainText: string, kind: number, scope: string) {
    ...
    if (!this.signer.nip44v3?.encrypt) {
      throw new Error('The extension you are using does not support nip44 v3 encryption')
    }
    return await this.signer.nip44v3.encrypt(pubkey, plainText, kind, scope)
  }
  ...
  async nip44v3Decrypt(pubkey: string, cipherText: string, kind: number, scope: string) {
    ...
    if (!this.signer.nip44v3?.decrypt) {
      throw new Error('The extension you are using does not support nip44 v3 decryption')
    }
    return await this.signer.nip44v3.decrypt(pubkey, cipherText, kind, scope)
  }
}
