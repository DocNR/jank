...
export default function BlossomServerListSetting() {
  ...
  useEffect(() => {
    const init = async () => {
      ...
      const event = await blossomServerListService.fetchBlossomServerListEvent(pubkey)
  ...
  const addBlossomUrl = async (url: string) => {
    ...
    try {
      ...
      await blossomServerListService.updateBlossomServerListEventCache(newEvent)
  ...
  return (
    <div className="space-y-2">
      ...
      {serverUrls.length === 0 && (
        <div className="bg-muted text-muted-foreground flex flex-col gap-1 rounded-lg border p-2 text-sm">
          ...
          <div className="flex flex-col">
            {RECOMMENDED_BLOSSOM_SERVERS.map((recommendedUrl) => (
              <Button
                ...
                className="text-muted-foreground hover:text-foreground h-fit w-fit p-0"
              >
                {recommendedUrl}
              </Button>
            ))}
