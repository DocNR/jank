...
class ThreadService {
  ...
  async subscribe(stuff: NostrEvent | string, limit = 100) {
    ...
    const _subscribe = async () => {
      ...
      if (rootPubkey) {
        const relayList = await relayListService.fetchRelayList(rootPubkey)
  ...
  async loadMore(stuff: NostrEvent | string, limit = 100): Promise<boolean> {
    ...
    const events = await timelineCache.loadMoreTimeline(timelineKey, subscription.until, limit)
    this.addRepliesToThread(events)
    ...
    const _parseRootInfo = async (): Promise<TRootInfo | undefined> => {
      let root: TRootInfo = event
        ? isReplaceableEvent(event.kind)
          ? {
              ...
              pubkey: event.pubkey,
              relay: seenOnService.getEventHint(event.id)
            }
      ...
      if (rootTag?.type === 'e') {
        ...
        if (rootEventHexId && rootEventPubkey) {
          ...
          if (rootEventId) {
            const rootEvent = await eventCache.fetchEvent(rootEventId)
            if (rootEvent) {
